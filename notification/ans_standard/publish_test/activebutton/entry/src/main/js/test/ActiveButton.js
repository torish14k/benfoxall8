...
describe('ActsAnsActionButtonTest', function () {
    ...
    function consumeCallback(err,data) {
        ...
        expect(typeof(data.actionButtons)).assertEqual('object')
        for(let i=0; i<data.actionButtons; i++)
        {
            console.debug("======consumeCallback====data.actionButtons.title:====>" + data.actionButtons[i].title);
            console.debug("======consumeCallback====sortings.hashCode:====>" + data.actionButtons[i].wantAgent);
            console.debug("======consumeCallback====sortings.hashCode:====>" + data.actionButtons[i].icon);
        }
    }
    //consume
    function consumeCallbackTwo(err,data) {
        ...
        for(let i=0; i<data.actionButtons; i++)
        {
            console.debug("======consumeDoNotCallbackTwo====sortings.hashCode:====>" + data.actionButtons[i].title);
            console.debug("======consumeDoNotCallbackTwo====sortings.hashCode:====>" + data.actionButtons[i].wantAgent);
            console.debug("======consumeCallback====sortings.hashCode:====>" + data.actionButtons[i].icon);
        }
    }
    //subscribeOn
    ...
    it('ActsActiveButton_test_0100', 0, async function (done) {
        ...
        var notificationRequest = {
            ...
            await notify.publish(notificationRequest, publishCallback200);
    ...
    it('ActsActiveButton_test_0200', 0, async function (done) {
        ...
        var subInfo ={
            onConsum:consumeCallbackTwo,
            onConnect:subscribeOnCallback,
        }
        try{
            await notify.subscribe(subInfo,subscribeCallback);
        }catch(err) {
            console.error('=ActsActiveButton_test_0200  订阅 activeButton err:'+err);
        }
        console.debug("===============ActsActiveButton_test_0200=======end===================>");
        ...
        console.info("===============agentInfo2========================>");
        var wantAgentDataOne = await wantagent.getWantAgent(agentInfoOne);
        var wantAgentDataTwo = await wantagent.getWantAgent(agentInfoTwo);
        ...
        var notificationRequest = {
            ...
            id: 1,
            slotType : notify.SlotType.SERVICE_INFORMATION,
            actionButtons: [{title:"button", wantAgent:wantAgentDataOne,icon:largeIcon},{title:"button", wantAgent:wantAgentDataTwo,icon:largeIcon}]
        }
        await notify.publish(notificationRequest, publishCallback200);
        console.info("===============ActsActiveButton_test_0200==========================>");
        done();
    })
})
