...
describe('XmlSerializerXmlPullParserTest', function () {
    ...
    it('testSetAttributes005', 0, function () {
        ...
        thatSer.endElement();
        var result = '<note importance1="high1" importance2="high2" importance3="high3" importance4="high4" importance5="high5"/>';
        var view = new Uint8Array(arrayBuffer);
        var view1 = "";
        for (var i = 0; i < result.length; ++i) {
            view1 = view1 + String.fromCodePoint(view[i]);
    ...
    it('testParse001', 0, function () {
        ...
        function func3(key, info){
            arr[i] = 'key:' + JSON.stringify(key) + info.getDepth() + info.getColumnNumber() + info.getLineNumber() + info.getAttributeCount() +
            info.getName() + info.getNamespace() + info.getPrefix() + info.getText() + info.isEmptyElementTag()+ info.isWhitespace();
            i++;
            return true;
        }
