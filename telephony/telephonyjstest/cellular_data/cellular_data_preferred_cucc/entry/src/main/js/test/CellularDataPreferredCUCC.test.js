 ...
 describe('Telephony_CellularDataPreferredCUCC', function () {
   ...
   it('Telephony_CellularData_activationDeactivation_Async_0700', 0, async function (done) {
     ...
     radio.setPreferredNetwork(SLOT_0, radio.PREFERRED_NETWORK_MODE_WCDMA, async function (err) {
       ...
       console.log(`${CASE_NAME} setPreferredNetwork finish`);
       await waitPreferredNetwork(CASE_NAME, radio.PREFERRED_NETWORK_MODE_WCDMA, TIME_DATA_RECONN);
       cellular.enableCellularData(async function (err) {
         if (err) {
           console.log(`${CASE_NAME} enableCellularData err: ${  err.message}`);
           expect().assertFail();
         ...
         let data = await stateChange(CASE_NAME, DataConnectState.DATA_STATE_DISCONNECTED, TIME_DATA_RECONN);
         expect(data === DataConnectState.DATA_STATE_DISCONNECTED).assertTrue();
         data = await stateChange(CASE_NAME, DataConnectState.DATA_STATE_CONNECTED, TIME_DATA_RECONN);
         expect(data === DataConnectState.DATA_STATE_CONNECTED).assertTrue();
